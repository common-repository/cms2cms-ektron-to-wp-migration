...
/**
 * Class CmsPluginFunctionsEktron
 */
class CmsPluginFunctionsEktron
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-ektron-to-wp-migration/reviews",
    ...
    {
        $cms2cms_ektron_access_login = get_option('cms2cms-ektron-login');
        $cms2cms_ektron_access_key   = get_option('cms2cms-ektron-key');

        return array(
            'email'     => $cms2cms_ektron_access_login,
            'accessKey' => $cms2cms_ektron_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_ektron_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-ektron-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-ektron-key', $key);

            delete_option('cms2cms-ektron-login');
            add_option('cms2cms-ektron-login', $login);

            delete_option('cms2cms-ektron-depth');
            add_option('cms2cms-ektron-depth', $bridge_depth);
    ...
    {
        return $this->__('Ektron', 'cms2cms-ektron-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-ektron-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-ektron-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-ektron-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if ( isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_ektron_logout')
                && $_POST['cms2cms_ektron_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
