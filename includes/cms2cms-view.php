<?php
if (!defined('CMS2CMS_EKTRON_VERSION')) {
    die();
}

$dataProvider = new CmsPluginFunctionsEktron();
$dataProvider->logOut();

$key              = get_option('cms2cms-ektron-key');
$activated        = $dataProvider->isActivated();
$targetUrl        = get_site_url();
$authentication   = $dataProvider->getAuthData();
$ajaxNonce        = $dataProvider->getFormTempKey('cms2cms-ektron-ajax-security-check');
$currentPluginUrl = plugin_dir_url(__FILE__);
$jsConfig         = $dataProvider->getConfig(true);
$config           = $dataProvider->getConfig();
$loader           = new EktronCmsBridgeLoader();

$styles = array($dataProvider->getFrontUrl() . 'css/cms2cms.css?v=' . CMS2CMS_EKTRON_VERSION);
foreach ($styles as $style) {
    printf('<link rel="stylesheet" href="%s" type="text/css" media="all">', $style);
}

$scripts = array(
    $dataProvider->getFrontUrl() . 'js/jsonp.js?v=' . CMS2CMS_EKTRON_VERSION,
    $dataProvider->getFrontUrl() . 'js/cms2cms.js?v=' . CMS2CMS_EKTRON_VERSION
);
...
?>
<div class="wrap cms2cms-ektron-wrapper">
    <div class="cms2cms-ektron-container">
        <div class="cms2cms-ektron-header">
        ...
        <?php if ($activated) { ?>
            <script language="JavaScript">
                jQuery('.cms2cms-ektron-container').addClass('cms2cms_ektron_is_activated');
            </script>
            <div class="cms2cms-ektron-message">
                <span>
                    <?php echo sprintf(
                        $dataProvider->__('You are logged in CMS2CMS as %s', $fileStart),
                        get_option('cms2cms-ektron-login')
                    ); ?>
                </span>
                <div class="cms2cms-ektron-logout">
                    <form action="" method="post" id="logout" data-logout="<?php echo $dataProvider->getLogOutUrl() ?>">
                        <input type="hidden" name="cms2cms_ektron_logout" value="1"/>
                        <input type="hidden" name="_wpnonce"
                               value="<?php echo $dataProvider->getFormTempKey('cms2cms_ektron_logout') ?>"/>
                        <button class="button-grey cms2cms-ektron-button" data-log-this="Logout" type="button">
        ...
        <?php } ?>
        <div class="cms2cms-ektron-plugin">
            <div id="cms2cms_ektron_accordeon">
                ...
                <?php
                $cms2cms_ektron_step_counter = 1;
                if (!$activated) { ?>
                    <div id="cms2cms_ektron_accordeon_item_id_<?php echo $cms2cms_ektron_step_counter++; ?>"
                         class="step-body cms2cms_ektron_accordeon_item cms2cms_ektron_accordeon_item_register">
                        ...
                        <form action="<?php echo $dataProvider->getLoginUrl() ?>"
                              ...
                              class="step_form"
                              id="cms2cms_ektron_form_register">
                            <div class="center-content">
                                <div class="error_message"></div>
                                <div class="user-name-block" style="display: none">
                                    <label
                                            for="cms2cms-ektron-user-name"><?php $dataProvider->_e('Full Name', $fileStart); ?></label>
                                    <input type="text" maxlength="50" id="cms2cms-ektron-user-name" name="name"
                                           value="" placeholder="Your name" class="regular-text"/>
                                    <div class="cms2cms-ektron-error name">
                                        <div class="error-arrow"></div>
                                        <span></span></div>
                                </div>
                                <div>
                                    <label
                                            for="cms2cms-ektron-user-email"><?php $dataProvider->_e('Email', $fileStart); ?></label>
                                    <input type="text" id="cms2cms-ektron-user-email" name="email"
                                           value="" placeholder="Your email" class="regular-text"/>
                                    <div class="cms2cms-ektron-error email">
                                        <div class="error-arrow"></div>
                                        <span></span></div>
                                </div>
                                <div>
                                    <label
                                            for="cms2cms-ektron-user-password"><?php $dataProvider->_e('Password', $fileStart); ?></label>
                                    <input type="password" id="cms2cms-ektron-user-password" name="password" value=""
                                           placeholder="Your password" class="regular-text"/>
                                    <div class="cms2cms-ektron-error password">
                                        <div class="error-arrow"></div>
                                        <span></span></div>
                                </div>

                                <input type="hidden" id="cms2cms-ektron-user-plugin" name="referrer"
                                       value="<?php echo $dataProvider->getPluginReferrerId(); ?>"
                                       class="regular-text"/>
                                <input type="hidden" id="cms2cms-ektron-site-url" name="siteUrl"
                                ...
                                <button
                                        ...
                                        type="button" id="auth_submit" class="cms2cms-ektron-button button-green">
                                ...
                                <a data-log-this="Forgot Password Link clicked"
                                   ...
                                   class="cms2cms-ektron-real-link" >
                ...
                <?php } /* cms2cms_ektron_is_activated */ ?>
                ...
                <div id="cms2cms_ektron_accordeon_item_id_<?php echo $cms2cms_ektron_step_counter++; ?>"
                     class="step-body cms2cms_ektron_accordeon_item">
                    <?php if (isset($message)) {?>
                        <div class="container-erorr-1">
                            <div class="block-erorr alert r-b-e">
                                <?php $dataProvider->_e(sprintf('%s Please contact our', $message), $fileStart); ?>
                    ...
                    <form action="<?php echo $dataProvider->getWizardUrl(); ?>"
                          method="post"
                          id="cms2cms_ektron_form_estimate">
                        <div class="center-content">
                            <div class="error_message"></div>
                            <div id='error'></div>
                            <div>
                                ...
                                <input type="text" id="sourceUrl" name="sourceUrl" value="" class="regular-text"
                                       ...
                                       ?>"/>
                                <div class="cms2cms-ektron-error">
                                    <div class="error-arrow"></div>
                                    <span></span></div>
                            </div>
                            ...
                            <input type="hidden" id="key" name="key"
                                   value="<?php echo $key; ?>"/>
                            <button id="verifySource_ektron"
                                    data-log-this="Verify Source Button pressed"
                                    type="button" name="verify-source" class="cms2cms-ektron-button button-green">
                                <?php $dataProvider->_e('Verify Connection', $fileStart); ?>
                            </button>
                        </div>
                    </form>
                    <div class="cms2cms-ektron-error"></div>
                </div>
            </div>
        </div> <!-- /plugin -->
